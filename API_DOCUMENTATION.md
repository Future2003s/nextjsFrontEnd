...
```json
{
  ...
  "data": {
    ...
    "refreshToken": "refresh_token",
    "expiresIn": 3600,
    "permissions": ["read", "write", "profile:update"]
  }
}
```
...
- `rememberMe` (boolean): Extend token expiration if true
- `deviceInfo` (object): For security tracking and session management
...
# Enhanced login with additional fields
curl -X POST http://localhost:8081/api/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{
    "email": "john@example.com",
